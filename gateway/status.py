...
def _get_process_start_time(pid: int) -> Optional[int]:
    ...
    if stat_path.exists():
        try:
            ...
            ticks = int(fields[21])
            # Convert kernel clock ticks to epoch seconds
            clock_ticks_per_sec = os.sysconf("SC_CLK_TCK")
            boot_time_ticks = 0
            # Read boot time from /proc/stat (btime line)
            try:
                for line in Path("/proc/stat").read_text().splitlines():
                    if line.startswith("btime"):
                        boot_time_ticks = int(line.split()[1])
                        break
            except (OSError, IndexError, ValueError):
                pass
            seconds_since_boot = ticks / clock_ticks_per_sec
            return int(boot_time_ticks + seconds_since_boot)
        except (IndexError, PermissionError, ValueError, OSError, ZeroDivisionError):
            pass
...
def _looks_like_gateway_process(pid: int) -> bool:
    ...
    On macOS, /proc is unavailable so we rely on ps/pgrep commands.
    """
    cmdline = _read_process_cmdline(pid)
    if not cmdline:
        # Fallback: check if the specific PID is alive AND looks like gateway
        try:
            os.kill(pid, 0)  # Check if process exists
        except (ProcessLookupError, PermissionError):
            return False
        # Try pgrep on macOS, but verify our target PID is in the results
        try:
            result = subprocess.run(
            ...
            if result.returncode == 0:
                running_pids = {int(p.strip()) for p in result.stdout.split() if p.strip().isdigit()}
